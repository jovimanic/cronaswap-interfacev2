...
import { ChevronDownIcon, LockClosedIcon, CalculatorIcon, SwitchVerticalIcon, ArrowDownIcon, ArrowUpIcon, PencilIcon } from '@heroicons/react/solid'
                ...
                <div className="flex flex-col justify-center w-2/12 space-y-1 lg:w-1/12">
                  ...
                  <div className="flex flex-col justify-center w-2/12 space-y-1 lg:w-2/12">
                    ...
                    <div className="items-center md:flex" >
                      <div className="text-xs font-bold md:text-base" onClick={() => setShowCalc(true)}>{formatPercent(farm.apr)}</div>
              ...
              <div className="pr-2 space-y-2 overflow-y-auto max-h-96 md:max-h-[480px] lg:max-h-[540px]">
                ...
                <div className='py-2'>
                  <div className='flex items-center justify-between text-sm'>
                    <Typography variant='sm'>{farm ?.name} {i18n._(t`LP Staked`)}</Typography>
                  ...
                  <div className="flex items-center justify-between mt-1 mb-2 rounded-md bg-dark-800">
                    <div className='w-11/12'>
                      <div className='flex items-end w-full p-3 space-x-3 focus:bg-dark-700'>
                        <Input.Numeric
                          ...
                          onUserInput={(val) => { editingCurrency==='usd'? (setUsdValue(Number(val)), setLpValue(Number(val)/farm?.lpPrice)): (setLpValue(Number(val)), setUsdValue(Number(val)*farm?.lpPrice)) }}
                        />
                    ...
                    <SwitchVerticalIcon className='w-1/12 mx-auto h-7' onClick={()=>{editingCurrency==='usd'? setEditingCurrency('lp'):setEditingCurrency('usd'); console.log("++++")}} />
                  </div>

                  <div className='flex items-center justify-between px-2 mb-2'>
                    <button className={`${buttonStyle} bg-dark-500 w-10 md:w-20`} onClick={() => {setUsdValue(100), setLpValue(100/farm?.lpPrice)}}>$100</button>
                    <button className={`${buttonStyle} bg-dark-500 w-10 md:w-20`} onClick={() => {setUsdValue(1000), setLpValue(1000/farm?.lpPrice)}}>$1000</button>
                    <button className={`${Lpbalance===0?buttonStyleDisabled: buttonStyle} bg-dark-500 w-20 md:w-24`} disabled={Lpbalance===0} onClick={() => { setLpValue(Lpbalance), setUsdValue(Lpbalance*farm.lpPrice)}}>My balance</button>
                    {/* Need to be fixed */}
                    <QuestionHelper text="“My Balance” here includes both LP Tokens in your wallet, and LP Tokens already staked in this farm." />
                  </div>

                  <Typography variant='sm'>{farm ?.name} {i18n._(t`Staked for`)}</Typography>
                  <div className="grid grid-cols-5 gap-2 mt-2 mb-4">
                    <button className={`${stakedPeriod===1? buttonStyleEnabled: buttonUnselected} px-2 mx-auto md:px-4`} onClick={() => setStakedPeriod(1)}>1D</button>
                    <button className={`${stakedPeriod===7? buttonStyleEnabled: buttonUnselected} px-2 mx-auto md:px-4`} onClick={() => setStakedPeriod(7)}>7D</button>
                    <button className={`${stakedPeriod===30? buttonStyleEnabled: buttonUnselected} px-2 mx-auto md:px-4`} onClick={() => setStakedPeriod(30)}>30D</button>
                    <button className={`${stakedPeriod===365? buttonStyleEnabled: buttonUnselected} px-2 mx-auto md:px-4`} onClick={() => setStakedPeriod(365)}>1Y</button>
                    <button className={`${stakedPeriod===365*5? buttonStyleEnabled: buttonUnselected} px-2 mx-auto md:px-4`} onClick={() => setStakedPeriod(365*5)}>5Y</button>
                  </div>

                  <Typography variant='sm'>{farm ?.name} {i18n._(t`Compounding Every`)}</Typography>
                  <div className="grid grid-cols-5 gap-2 mt-2 mb-3">
                    <div className="m-auto">
                      <Checkbox checked={isCompounding} className='items-end justify-center w-8 h-8' color='blue' set={() => setIsCompounding(!isCompounding)} />
                    </div>
                    <button className={`${isCompounding ? (compoundingPeriod===1? buttonStyleEnabled:buttonUnselected) : buttonStyleDisabled} px-2 mx-auto md:px-4`} disabled={!isCompounding} onClick={() => setCompoundingPeriod(1)}>1D</button>
                    <button className={`${isCompounding ? (compoundingPeriod===7? buttonStyleEnabled:buttonUnselected) : buttonStyleDisabled} px-2 mx-auto md:px-4`} disabled={!isCompounding} onClick={() => setCompoundingPeriod(7)}>7D</button>
                    <button className={`${isCompounding ? (compoundingPeriod===14? buttonStyleEnabled:buttonUnselected) : buttonStyleDisabled} px-2 mx-auto md:px-4`} disabled={!isCompounding} onClick={() => setCompoundingPeriod(14)}>14D</button>
                    <button className={`${isCompounding ? (compoundingPeriod===30? buttonStyleEnabled:buttonUnselected) : buttonStyleDisabled} px-2 mx-auto md:px-4`} disabled={!isCompounding} onClick={() => setCompoundingPeriod(30)}>30D</button>
                  </div>
                  <ArrowDownIcon className='h-6 mx-auto mb-3' />

                  <div className="p-2 space-y-1 rounded-lg bg-dark-800">
                    <Typography variant='sm' className='text-cyan-blue'>{i18n._(t`ROI AT CURRENT RATES`)}</Typography>
                    <div className="h-6 max-w-full text-lg font-bold text-blue md:text-xl">$ {ROIcalculator(usdvalue, farm?.apr).ROI.toFixed(2)}</div>
                    <div className="text-sm">~ {ROIcalculator(usdvalue, farm?.apr).ROIInTokens} CRONA ({ROIcalculator(usdvalue, farm?.apr).ROIPercentage}%)</div>
                  </div>
                </div>
                ...
                <div className="align-middle bg-dark-800">
                  <div className="flex items-center justify-center mx-auto">
                    ...
                    {isExpand ? <ArrowUpIcon className="h-4 text-cyan-blue" /> :
                  ...
                  {isExpand &&
                    <div className="p-2 text-sm h-fit md:text-base">
                      ...
                      <div className="my-2 text-sm list-style-type:disc;">
                        ...
                        <Typography variant='sm'>{i18n._(t`• LP rewards: 0.17% trading fees, distributed proportionally among LP token holders.`)}</Typography>
                        <Typography variant='sm'>{i18n._(t`• All figures are estimates provided for your convenience only, and by no means represent guaranteed returns.`)}</Typography>
                      </div>
                      <ExternalLink
