import { Binance, ChainId, Currency, Ether, Cronos, Token, WNATIVE } from '@cronaswap/core-sdk'
...
export default function Bridge() {
  ...
  const [chainTo, setChainTo] = useState<Chain | null>(
    chainId == ChainId.CRONOS ? DEFAULT_CHAIN_FROM : DEFAULT_CHAIN_TO
  )
  ...
  const { data: anyswapInfo, error }: SWRResponse<AnyswapTokensMap, Error> = useSWR(
    'https://bridgeapi.anyswap.exchange/v2/serverInfo/25',
    (url) =>
      fetch(url)
    ...
    let tokens: Currency[] = Object.keys((anyswapInfo && anyswapInfo[chainFrom.id]) || {})
      ...
      .map((r) => {
        ...
        if (r.toLowerCase() == WNATIVE[chainFrom.id].address.toLowerCase()) {
          if (chainFrom.id == ChainId.CRONOS) {
            return Cronos.onChain(chainFrom.id)
          }
  ...
  const handleChainFrom = useCallback(
    (chain: Chain) => {
      ...
      if (changeTo.id !== ChainId.CRONOS && chain.id !== ChainId.CRONOS) {
  ...
  const handleChainTo = useCallback(
    (chain: Chain) => {
      ...
      if (changeFrom.id !== ChainId.CRONOS && chain.id !== ChainId.CRONOS) {
  ...
  const bridgeToken = async () => {
    ...
    try {
      if (currency0.chainId == ChainId.CRONOS) {
  ...
  return (
    <>
      ...
      <Head>
        <title>{i18n._(t`Bridge`)} | CronaSwap</title>
