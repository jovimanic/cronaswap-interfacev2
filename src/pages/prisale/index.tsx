...
export default function Prisale() {
  ...
  const tokenBalanceBignumber = useTokenBalance(account ?? undefined, prisaleToken[chainId])
  const tokenBalance = Number(tokenBalanceBignumber?.toSignificant(8))

  const prisaleContract = usePrivateSaleContract()

  const parsedStakeAmount = tryParseAmount(investValue, tokenBalanceBignumber?.currency)
  ...
  const handleBuyToken = async () => {
    setPendingTx(true)
    try {
      const args = [USDC[chainId].address, Number(investValue) * 10 ** USDC[chainId].decimals]
      console.log('+++', args)
  ...
  return (
    <div className="w-9/12 mt-20">
      ...
      <div className="py-5 rounded-lg px-7 bg-black-russian">
        ...
        <div className="my-7">
          <ProgressBar progress={0.3235} className="from-green to-carribean-green" />
        </div>
          ...
          <div className="px-5 py-9 border-[1px] border-gray-700 rounded-xl space-y-4 md:w-1/2">
            <div className="flex justify-between gap-3">
              <div className="rounded-lg border-[1px] border-gray-700 py-6 px-4 w-1/2">
                ...
                <div className="text-sm ">{purchasedToken.current}</div>
              </div>
              <div className="rounded-lg border-[1px] border-gray-700 py-6 px-4 w-1/2">
                <div className="text-base text-white">{i18n._(t`Claimable ${prisaleToken[chainId].symbol}`)}</div>
                <div className="text-sm ">{claimableToken.current}</div>
              </div>
            </div>
