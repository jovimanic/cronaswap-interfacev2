...
export const usePriceCronaUSDC = (): BigNumber => {
  const cronaCROFarm = useFarmFromPid(1)

  const cronaPriceUSDCAsString = cronaCROFarm.tokenPriceUSDC

  const cronaPriceUSDC = useMemo(() => {
    return new BigNumber(cronaPriceUSDCAsString)
  }, [cronaPriceUSDCAsString])

  return cronaPriceUSDC
}
